...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
        ...
        self.cate_idx_lst = []
        self.all_points = []
        for cate_idx, subd in tqdm(enumerate(self.subdirs), total=len(self.subdirs)):
...
class MNIST_CP(Dataset):
    def __init__(self, root_dir, tr_sample_size, te_sample_size, categories=['0'],
        ...
        # categories can be from 0 to 9
        for cate_idx, subd in tqdm.tqdm(enumerate(self.categories), total=len(self.categories)):
            # NOTE: [subd] here is synset id
            sub_path = os.path.join(root_dir, subd, self.split)
            if not os.path.isdir(sub_path):
                print("Directory missing : %s" % sub_path)
...
def get_data(cfg, idx, split='train'):

    dataset = ShapeNet15kPointClouds(
        categories=cfg.cates, split=split,
...
def get_data_mnist_cp(cfg, idx, split='train'):
    
    dataset = MNIST_CP(
        root_dir=cfg.data_dir, categories=cfg.cates, tr_sample_size=cfg.tr_max_sample_points, 
        te_sample_size=cfg.te_max_sample_points, split=split, scale=cfg.dataset_scale, 
        normalize_per_shape=False, random_subsample=True, normalize_std_per_axis=cfg.normalize_std_per_axis, 
        recenter_per_shape=cfg.recenter_per_shape
    )
