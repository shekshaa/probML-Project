...
class EBM(nn.Module):
    ...
    def forward(self, x, c):
        ...
        for block in self.blocks:
            net = block(net, c_xyz)
        out = self.conv_out(self.actvn_out(self.bn_out(net))).transpose(1, 2)  # shape: (bs, num_points, 3)
        return self.lin_out(self.actvn_out(out)).squeeze(-1)  # shape: (bs, num_points)