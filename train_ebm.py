...
from utils import keep_grad, approx_jacobian_trace, exact_jacobian_trace, \
    set_random_seed, get_logger, dict2namespace, get_opt, visualize, langevin_dynamics_ebm
import importlib
import argparse
...
def parse_args():
    ...
    parser.add_argument('--cfg', type=str, default='test_config_ebm.yaml',
...
def train(args):
    ...
    opt_ebm, scheduler_ebm = get_opt(ebm.parameters(), cfg.trainer.opt_ebm)
    opt_criticnet, scheduler_criticnet = get_opt(critic_net.parameters(), cfg.trainer.opt_ebm)
    ...
    for epoch in range(cfg.trainer.epochs):
        for data in train_loader:
            ...
            logp_u = ebm(perturbed_points, used_sigmas)
            score_pred = keep_grad(logp_u.sum(), perturbed_points)
            ...
            if itr % cfg.log.viz_freq == 0:
                ...
                pt_cl, _ = langevin_dynamics_ebm(ebm, sigmas, eps=1e-4, num_steps=cfg.inference.num_steps)
